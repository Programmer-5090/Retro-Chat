...
pub fn format_spectrum_bars(
    ...
    if width == 0 || rows == 0 || bins.is_empty() {
        return Vec::new();
    }
...
pub fn format_idle_waveform(msg_id: &str, width: usize, color: Color) -> Line<'static> {
    if width == 0 {
        return Line::from("");
    }
    let mut hasher = DefaultHasher::new();
...
fn lerp_color3(a: Color, b: Color, c: Color, t: f32) -> Color {
    if t < 0.5 { lerp_color(a, b, t * 2.0) } else { lerp_color(b, c, (t - 0.5) * 2.0) }
}
